...
class IvaAnalyzerService:
    ...
    def calculate_trimestral_iva_by_user_id(self, user_id, year:int = None):
        ...
        deducted_expenses = self.user_query_service.retrieve_user_with_deducted_expenses_map_list(user_id, year=year)
        invoices = self.user_query_service.retrieve_user_with_invoices_map_list(user_id, include_unpaid_invoices=False, year=year)
    ...
    def calculate_tot_trimestral_iva(self, year:int = None):
        ...
        for user in self.user_query_service.retrieve_users_map_list():
            if user[DBUsersColumns.REGIME_FISCALE.value] == RegimeFiscale.ORDINARIO.value:
                ...
                output_map[user_name] = self.calculate_trimestral_iva_by_user_id(user_id, year=year)
