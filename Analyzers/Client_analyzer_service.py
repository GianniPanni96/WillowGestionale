...
class ClientAnalyzerService:
    ...
    def calcola_tot_entrate_cliente(self, client_id, include_unpaid_invoices: bool = True, year: int = None):
        client_with_invoices = self.client_query_service.retrieve_client_with_invoices_map_list(client_id,
        ...
        for row in client_with_invoices:  # in questo modo sto in realtà scorrendo le fatture
            if (row[DBInvoicesColumns.TIPO.value] != TipologiaFattura.NOTA_DI_CREDITO.value or
                ...
                tot = tot + float(row[DBInvoicesColumns.TOT_DOCUMENTO.value]) if row[
                                                                                     DBInvoicesColumns.TOT_DOCUMENTO.value] is not None else tot
    ...
    def calcola_numero_fatture_cliente(self, client_id, include_unpaid_invoices: bool = True, year: int = None):
        ...
        for row in client_with_invoices:
            ...
            if (row[
                DBInvoicesColumns.TIPO.value] != TipologiaFattura.NOTA_DI_CREDITO.value and valid_row or
                    row[
                        DBInvoicesColumns.STATUS.value] != InvoiceSatus.STORNATA.value and valid_row):
                tot = tot + 1

        return tot

    def calcola_media_fatture_cliente(self, client_id, include_unpaid_invoices: bool = True, year: int = None):
        numero = self.calcola_numero_fatture_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices,
                                                     year=year)
        tot = self.calcola_tot_entrate_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices,
                                               year=year)
    ...
    def calcola_pagam_orario_medio_cliente(self, client_id, include_unpaid_invoices: bool = True, year: int = None):
        ...
        for invoice in filtered_maps:
            if invoice[DBInvoicesColumns.ID_CLIENTE.value] == client_id:
                tot_pagam = tot_pagam + float(invoice[
                                                  DBInvoicesColumns.TOT_DOCUMENTO.value])  # legito se esiste una sola produzione per fattura, altrimenti sommo più volte la stessa fattura
    ...
    def construct_client_map_aggregate_data(self, client_id, include_unpaid_invoices:bool = True, year:int = None):
        ...
        client_aggregate_data = {
            ClientsAggregateData.TOT_ENTRATE.value: self.calcola_tot_entrate_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.NUM_FATTURE.value: self.calcola_numero_fatture_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.MEDIA_FATTURE.value: self.calcola_media_fatture_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.TOT_CREDITI.value: self.calcola_totale_crediti_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.TOT_RIMBORSI.value: self.calcola_tot_rimborsi_by_client(client_id, year=year),
            ClientsAggregateData.PAGAM_ORARIO_MEDIO.value: self.calcola_pagam_orario_medio_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.TOT_GIORNI_RIT.value: self.calcola_totale_ritardi_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year),
            ClientsAggregateData.MEDIA_RITARDO.value: self.calcola_ritardo_medio_cliente(client_id, include_unpaid_invoices=include_unpaid_invoices, year=year)
        }
