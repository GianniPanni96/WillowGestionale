...
class BackupScheduler:
    ...
    def _backup_gestionale_db(self, n, backup_base_path, delta_days):
        """
        ...
        if os.path.exists(interval_folder):
            ...
            # Se il numero di backup è maggiore di n, elimina i più vecchi
            while len(subfolders) > n:
...
class BackupImporter:
    ...
    Manages listing and importing backups stored under:
      backup_base_path/<interval_folder>/<subfolder_with_timestamp>/
    ...
    def __init__(self, backup_base_path: str, db_path: str):
        self.backup_base_path = os.path.abspath(backup_base_path) if backup_base_path else None
        # db_path può essere la cartella contenente gestionale.db oppure il path completo del file gestionale.db
    ...
    def _collect_all_subfolders(self) -> List[str]:
        """
        Ritorna una lista di percorsi assoluti per ogni subfolder trovato in backup_base_path/*/*
        (cioè scende di due livelli: interval_folder -> subfolders).
        """
        results = []
        if not self.backup_base_path or not os.path.isdir(self.backup_base_path):
            return results

        try:
            for interval in os.listdir(self.backup_base_path):
                interval_path = os.path.join(self.backup_base_path, interval)
    ...
    def list_backups_for_year(self, year: int) -> List[Dict]:
        ...
        for p in candidates:
            ...
            if dt.year == year:
                display = dt.strftime("%Y-%m-%d %H:%M:%S") + " — " + os.path.relpath(p, self.backup_base_path)
